...
import { formatNumberShort, formatRelativeTime } from "@/utils/formatters";
    ...
    <View className="border-b border-gray-100 bg-white">
      <View className="flex-row p-4">
        ...
        <View className="flex-1">
          ...
          {post.image ?(
            <Image
              source={{ uri: post.image || "https://via.placeholder.com/400x300" }}
          ...
          <View className="flex-row justify-between max-w-xs">
            <TouchableOpacity className="flex-row items-center" onPress={() => onComment(post)}>
              ...
              <Text className="text-gray-500 text-sm ml-2">
                {formatNumberShort(post.comments?.length || 0)}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity className="flex-row items-center" onPress={() => onLike(post._id)}>
              ...
              <Text className={`text-sm ml-2 ${isLiked ? "text-red-500" : "text-gray-500"}`}>
                {formatNumberShort(post.likes?.length || 0)}
              </Text>
            </TouchableOpacity>
